...
class Config:
    ...
    def load_config(self):
        ...
        try:
            ...
            print(
                "Error decoding config file. Creating backup and starting with defaults."
            )
            # Create backup of corrupted file
            backup_file = self.config_file.with_suffix(".json.corrupted")
            if self.config_file.exists():
                import shutil
            ...
            print(
                f"Error loading config: {e}. Creating backup and starting with defaults."
            )
            # Create backup of problematic file
            backup_file = self.config_file.with_suffix(f".json.error_backup")
            if self.config_file.exists():
                import shutil
                ...
                print(f"Backup saved to: {backup_file}")
            self.config = DEFAULT_CONFIG.copy()
            # Save the default config to recover from the error
            self.save_config()
        ...
        for i, profile in enumerate(connections):
            if profile.get("name") == name:
                ...
                if new_name != name and self.get_supabase_connection_by_name(new_name):
                    raise ValueError(
                        f"Cannot rename profile to '{new_name}', name already exists."
                    )
